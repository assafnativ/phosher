...
TOKENS = {
        ...
        0xC3: 'OPAQUE',
        0xC4: 'LITERAL_AC'}
TOKENS_REVESRE = dict([(y, x) for x, y in TOKENS.items()])
...
class WBXML(ObjectWithStream):
    ...
    def parseToken(self, codePage=0, isAttrib=False):
        ...
        if tag in TOKENS:
            ...
            if 'STR_I' == token:
                ...
                self.currentCodePage = self.readByte()
    ...
    def resolveFromDB(self, tagId, isAttrib):
        if isAttrib:
            ...
            subDic = "tag"
        if self.publicIdName in VALUES_DB:
            db = VALUES_DB[self.publicIdName][subDic]
        else:
            return str(tagId)
    ...
    def parse(self):
        ...
        self.publicIdName = PUBLIC_ID.get(self.publicId, '?')
...
def textifyAllWBXMLs(path):
    for fname in glob(path):
        if os.path.isdir(fname):
            print "Scanning subdir: %s" % fname
            textifyAllWBXMLs(fname + os.sep + '*')
        elif fname.lower().endswith('wbxml'):
            w = WBXML(fname)
            outputFName = fname + '.txt'
            print "Textifing: %s -> %s" % (fname, outputFName)
            file(outputFName, 'wb').write(w.display())

def main():
    if len(sys.argv) != 2:
        print "Usage error: python wbxml.py WBXML_file"
        return
    wbxmlFile = sys.argv[1]
    textifyAllWBXMLs(wbxmlFile)
